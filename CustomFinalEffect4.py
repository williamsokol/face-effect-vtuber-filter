...
DARKNESS_THRESHOLD = 25     # Pixels below this brightness will be treated as the raw RGB area
...
while cap.isOpened():
    ...
    if segmentation_result.category_mask:
        ...
        # --- Create Base Masks ---
        brightness_factor = 0.7  # 70% of original brightness
        ...
        # Use values between 0.0 (black) and 1.0 (original brightness)
        brightness_factor2 = .6  # 70% of original brightness
        dark_rgb = (rgb_frame * brightness_factor2).astype(np.uint8)
    ...
    # 5. Perform Face Tracking on the RGB frame (not final_image_rgb)
    final_image_rgb.flags.writeable = False
    
    # Process the RGB frame for accurate landmark placement
    face_results = face_mesh.process(rgb_frame) 
    
    # Draw the face mesh annotations onto the final image
    final_image_rgb.flags.writeable = True

    if face_results.multi_face_landmarks:
        for face_landmarks in face_results.multi_face_landmarks:
            eyebrow_style = mp_drawing.DrawingSpec(color=(0, 0, 0), thickness=2, circle_radius=1)  # Black
            eye_style = mp_drawing.DrawingSpec(color=(0, 0, 0), thickness=2, circle_radius=10)  # Black
            
            # Draw only specific contours you want (uncomment the ones you need):
    ...
    # 5.5. Darken the RGB image after face landmarks are drawn
